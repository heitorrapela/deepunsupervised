...
import metrics
from models.autoencoder import AutoEncoderSOM
...
def train_autoencoder_model(root, dataset_path, parameters, device, use_cuda, out_folder, debug, n_samples,
    ...
    for param_set in parameters.itertuples():
        ...
        model = AutoEncoderSOM(d_in=dataset.d_in,
                    hw_in=dataset.hw_in,
        ...
        for epoch in range(param_set.epochs):
            ...
            for batch_idx, (sample, target) in enumerate(train_loader):
                ...
                decoded_features, som_output  = model(sample)
                samples_high_at, weights_unique_nodes_high_at, relevances = som_output
                ...
                if len(samples_high_at) > 0:
                    ...
                    loss_som = weightedMSELoss(samples_high_at, weights_unique_nodes_high_at, relevances)
                    # som_loss.backward()
                    # optimizer.step()
                else:
                    loss_som = 0.0
                ...
                loss_autoencoder = mseLoss(sample, decoded_features)

                # ===================backward====================
                alpha = 0.2  # alpha = 0.8
                print("Reconstruction Loss:", loss_autoencoder)
                print("Clustering Loss:", loss_som)
                loss = (1 - alpha) * loss_autoencoder + alpha * loss_som
